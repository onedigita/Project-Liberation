...
_cancel = player addAction [_str, "scripts\client\CAS\casMenu.sqf", nil, -1, false, true, ""];
...
if (alive player && _num > 0) then
{
  ...
  player addAction [_str, "scripts\client\CAS\casMenu.sqf", [_maxDist, _lock, _num], -1, false, true, ""];
};
