...
while{true}do{
   ...
   // PLAYER NAME CHECK AND DISPLAY
        ...
        if (_target isKindOf "Man" && player == vehicle player) then{
                if((side _target == playerSide) && (player distance _target) < 20)then{
	...
	// PRINT THE RANK/NAME/WEAPON ONSCREEN
						...
						_nameString = format ["<t size='0.5' color='#f0e68c'>%4 </t><t size='0.5' color='#f0e68c'>%1</t><br/><t size='0.5' color='#f0e68c'>%2</t><br/><img size='0.8' image='%3'/><br/>",_name];
